import { Pool } from "pg";
// Direct Supabase endpoint (5432) instead of the transaction-mode pooler (6543)
const url = (process.env.SUPABASE_DATABASE_URL_B64
  ? Buffer.from(process.env.SUPABASE_DATABASE_URL_B64, "base64").toString()
  : process.env.DATABASE_URL).replace("aws-0-ap-southeast-2.pooler.supabase.com", "db.oaxgmvsxzfkyqzmfwxtn.supabase.co");
console.log("using:", url.replace(/:.*@/, ":REDACTED@"));
const pool = new Pool({ connectionString: url, max: 8, min: 1, idleTimeoutMillis: 30000, keepAlive: true, keepAliveInitialDelayMillis: 10000, ssl: { rejectUnauthorized: false } });
