...
export const systemRouter = router({
  ...
  // TEMPORARY (migration): admin-only procedure that creates the Postgres schema
  // and migrates all data from the MySQL source into Supabase. Remove after migration.
  supabaseMigrate: adminProcedure.mutation(async () => {
    const url =
      (process.env.SUPABASE_DATABASE_URL_B64
        ? Buffer.from(process.env.SUPABASE_DATABASE_URL_B64, "base64").toString("utf8")
        : "") || process.env.SUPABASE_DATABASE_URL || "";
